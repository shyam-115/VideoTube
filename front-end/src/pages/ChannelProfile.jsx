        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-0">
          <div className="flex flex-col md:flex-row items-center md:space-x-6 text-center md:text-left gap-3 md:gap-0">
            <img
              ...
              className="w-24 h-24 md:w-28 md:h-28 rounded-full object-cover ring-4 ring-white dark:ring-zinc-950 shadow-lg"
            />
            <div>
              <h1 className="text-2xl md:text-4xl font-extrabold text-zinc-900 dark:text-white tracking-tight leading-tight">{channel.fullname}</h1>
              <p className="text-zinc-500 dark:text-zinc-400 text-base md:text-lg font-medium mt-1">@{channel.username}</p>
              <p className="text-zinc-500 dark:text-zinc-400 text-sm md:text-base font-medium mt-1 md:mt-2">{subscriberCount} subscribers • {videos.length} videos</p>
            </div>
          </div>
          ...
          {user && user._id !== channel._id && (
            <button
              ...
              className={`relative w-full md:w-auto inline-flex items-center justify-center px-8 py-3.5 text-lg font-semibold text-white rounded-xl transition-all shadow-sm active:scale-95 mt-2 md:mt-0 ${
                isSubscribed
                  ? 'bg-zinc-200 text-zinc-800 md:hover:bg-zinc-300 dark:bg-zinc-700 dark:text-zinc-200 dark:md:hover:bg-zinc-600'
                  : 'bg-gradient-to-r from-violet-600 to-indigo-600 md:hover:from-violet-500 md:hover:to-indigo-500 shadow-lg shadow-violet-500/25'
              }`}
            >
