  ...
  return (
    <div className="p-4">
      ...
      <div className="rounded-3xl border border-zinc-200/50 p-6 mb-10 bg-white/70 backdrop-blur-xl dark:bg-zinc-900/70 dark:border-zinc-800/50 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-20 flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-0">
          <div className="flex flex-col md:flex-row items-center md:space-x-6 text-center md:text-left gap-3 md:gap-0">
            <img
              src={user.avatar}
              alt={user.username}
              className="w-24 h-24 md:w-28 md:h-28 rounded-full object-cover ring-4 ring-white dark:ring-zinc-900 shadow-lg"
            />
            <div>
              <h1 className="text-2xl md:text-4xl font-extrabold text-zinc-900 dark:text-white tracking-tight leading-tight">{user.fullname}</h1>
              <p className="text-zinc-500 dark:text-zinc-400 text-base md:text-lg font-medium mt-1">@{user.username}</p>
              <p className="text-zinc-500 dark:text-zinc-400 text-sm md:text-base font-medium mt-1 md:mt-2">{subscriberCount} subscribers • {myVideos.length} videos</p>
            </div>
          </div>
          <button
            onClick={handleEditClick}
            className="relative w-full md:w-auto inline-flex items-center justify-center p-[2px] overflow-hidden text-sm font-semibold text-zinc-900 dark:text-white rounded-xl group bg-gradient-to-br from-zinc-200 to-zinc-300 hover:from-zinc-300 hover:to-zinc-400 dark:from-zinc-800 dark:to-zinc-700 dark:hover:from-zinc-700 dark:hover:to-zinc-600 transition-all duration-300 shadow-sm active:scale-95 mt-2 md:mt-0"
          >
            <span className="relative w-full md:w-auto justify-center px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-zinc-900 rounded-[10px] group-hover:bg-opacity-0 flex items-center">
